...
// ─── Component ────────────────────────────────────────────────────────────────
export const ChatPanel = ({ settings, onOpenSettings, onSettingsChange }) => {
    ...
    const handleAgentSend = useCallback(async (text, contextText) => {
        ...
        try {
            ...
            }, rootDir || undefined, currentSettings, 200, controller.signal);
        }
        catch (err) {
            if (err instanceof Error && err.name !== 'AbortError') {
