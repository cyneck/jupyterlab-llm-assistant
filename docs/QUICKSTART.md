...
v0.7.0 将 Chat / Agent / Plan 三种模式合并到**同一个面板**。底部的输入区域始终可见，左下角下拉框用于在三种模式间切换。
...
Plan 模式适合需要拆解为多步骤的复杂任务。

1. 在底部下拉框选择 **Plan**
2. 输入高层次任务描述，发送后 LLM 自动生成步骤列表
3. 可编辑各步骤的标题和描述
4. 点击"Run All"依次执行全部步骤，或"Run Step"逐步执行
5. 每步执行结果（文本 + 工具调用）在卡片中展开显示
